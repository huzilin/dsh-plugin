...
description: >-
  ...
  or to build test cases / test-suite assets for a feature/API/flow/page. 跑用例/执行/验收/回归/
...
| 输入 | 何时有 / 用于什么 |
|---|---|
| `prd`（product_doc，to-prd 产出）§5 功能需求含 AC | AC → 验收用例逐条映射（B/C 组主骨架） |
| spec / 架构文档全文 | 抽「行为性明文」清单（状态机、联动条款、「X 时必须 Y」）——**不只 AC 章** |
| 契约真相源（api proto / openapi） | A 组接口专项：每接口正常/拦截/幂等/负路径 |
| `rd_design` / `fe_design`（to-rd-techdesign / to-fe-techdesign 产出） | 接口口径、数据可测性、边界态说明 → 联调场景与数据构造 |
...
2. **用例设计（cases.md）**：按六组结构落用例，每条唯一编号 `组-序号`，预期结果可判定（禁「功能正常」）：
...
- 不生成需求或 AC（那是 to-brd / to-prd 的活）；AC 缺失走 gap-naming 并批注，不自造验收口径。
