...
| 输入 | 何时有 / 用于什么 |
|---|---|
| `prd`（产品需求文档）§5 功能需求含 AC | AC → 验收用例逐条映射（B/C 组主骨架） |
| spec / 架构文档全文 | 抽「行为性明文」清单（状态机、联动条款、「X 时必须 Y」）——**不只 AC 章** |
| 契约真相源（api proto / openapi） | A 组接口专项：每接口正常/拦截/幂等/负路径 |
| `rd_design` / `fe_design`（研发 / 前端技术设计文档） | 接口口径、数据可测性、边界态说明 → 联调场景与数据构造 |
...
|---|---|---|
| prd §5 AC | 每条 AC 的通过/不通过判据 | §2 覆盖矩阵「覆盖的 AC」列（B/C 组主骨架） |
| spec 行为性明文 | 状态迁移、跨表联动、通知触发 | B 组断言维度（含跨层联动 sweep） |
| 契约 | 每端点入参/出参/错误码 | A 组（每接口至少 1 正 1 负 1 幂等） |
| rd/fe design | 数据可测性、边界态、序列化口径 | §4 联调场景与数据构造；资产 env/seed |
