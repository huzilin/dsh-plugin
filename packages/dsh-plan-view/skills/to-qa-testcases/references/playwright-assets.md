...
> 本文件是**有 UI 项目呈现通道可执行资产**形态的唯一权威。SKILL.md 与 testsuite-skeleton.md 只放指针，不复述本文。
...
> **等待口径**：spec 内「等某事完成」一律条件探测——自动重试断言或 `expect.poll`（探测间隔+超时上限），裸 `waitForTimeout` 作就绪判定 = 违例；红线正本 `testsuite-skeleton.md` §二「等待与确定性红线」。
> **并发与环境口径**：端口/工作目录/结果落点一律带测例集标识、零共享可并发；`qa/results/`、`qa/screenshots/` 本落仓库内，临时物禁 `/tmp`；红线正本 `testsuite-skeleton.md` §二「并发与环境红线」。
...
- **视觉维记录 = 既有三态「未覆盖 + 附注（视觉维留证待审）」，不出「通过」**。这是防「接口通路但页面崩了」类假阳性的硬要求（见 acceptance-dod.md 五维 DoD）。不新造第四态状态词。
