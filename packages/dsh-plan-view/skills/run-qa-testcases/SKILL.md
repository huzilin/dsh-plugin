...
1. **取测例与资产**：读该图 `cases.md` §2/§3 取本轮要跑的用例编号；**跑批前核数据集来源**（须为 cases.md §0 三件套声明的本集自建数据集；他人造数与验收专用数据不可消费——八源盘点第 8 源「环境与数据纪律」）；`env_up` 重建测试环境；按 cases.md §0「测试代码」行声明的驱动入口，确认驱动脚本可按用例编号过滤单跑。测例集缺失或不可执行 → 退回 `to-qa-testcases`，不在本 skill 现造用例。
