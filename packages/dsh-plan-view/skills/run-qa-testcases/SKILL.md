...
输入 = `to-qa-testcases` 产出的 `.plan/<effort>/qa/cases.md` 与仓库 `qa/` 可执行资产（env_up/env_down、驱动脚本、诊断最小环）。本 skill 跑测例 → 出验收记录 → 出缺陷台账 → 返工闭环 → 更新回归基线。产物落 `.plan/<effort>/qa/test.md` + `.plan/<effort>/qa/defect.md` + `qa/screenshots/`（可执行脚本留仓库 `qa/`）。
...
2. **双通道执行**：
   ...
   - **呈现通道**：浏览器真实走完用户旅程（有 FE 必走），P 组逐元素位执行，**截图逐条落 `qa/screenshots/`**（含整页首屏，文案类截图须能看清文字），对照原型逐条 diff；
   - **红线：纯协议通道不得对 UI 出「通过」**；
...
4. **缺陷台账（defect.md）**：按骨架补**真 frontmatter 状态头**（`type: qa-defect` + 四字段，围栏/引用块=插件读不到）；每条缺陷 A~E 五节登记，**E 节「最小复现入口」必填**（一条命令 + 环境变量 + 预期红信号，见「交接契约」）；总览表头九列逐字照骨架；登记**发现源**（QA 轮 / 用户）与**测试设计缺口**——用户渠道缺陷必填缺哪个源/维度/通道，缺口**当轮**补进 cases.md 并按案例库格式追加 `to-qa-testcases/references/case-library.md`。

5. **返工闭环**：缺陷按**类型**（`rd` / `fe` / `arch` / `docs`，既有 role_id）派发 → 修复动作交 `diagnosing-bugs`（按 E 节契约，修复后只回写 C 节）→ **用第 1 步资产复测**（不手工重验）→ 复测 PASS 登记关闭人/日期（状态附注如「已关闭（复测 PASS）」）→ 更新回归基线；wbflow 环境下对应 `wb_comment(domain)` 路由 → resolve → confirm/reopen。

6. **落盘 + 自检**：test.md / defect.md / screenshots/ 落 `.plan/<effort>/qa/` → 按本 skill「自检」逐条过 → 登记产物（wbflow：`wb_add_artifact`，nodeId=当前节点）→ 台账全部关闭时把 frontmatter `status` 翻 `closed`（仍有未关闭保持 `active`）→ 提评/推进。
...
- **full 分账**：各图结果写回各自 `qa/test.md`，新缺陷进各自 `qa/defect.md`；
...
- [ ] （blocker）defect.md 每条缺陷 E 节「最小复现入口」齐备且末次运行为红
