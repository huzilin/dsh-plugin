...
- **票 04 done（2026-09-22）**：Playwright 接入协议改票——新建形态正本 `playwright-assets.md`（D5 六项 + DOM 探针清单 + 无视觉三态口径 + VLM 位置表）、to 侧指针 / run 侧四处同步改、qa-records-skeleton §3 视觉维附注；施工中回补源仓落后装态的漂移（09-21 拍板版），三侧 `diff -rq` 逐字一致。裁决源 `.plan/待拍板-to-qa-testcases接入playwright-20260921.md`（closed）；词汇正本 `CONTEXT.md`（五词）。**已 commit+push（`204bf70`）并部署（部署副本 skills 同步一致）**；novel 脚手架待立项。见 [tickets/04](tickets/04-playwright-protocol-assets.md)。
